...
export class SessionsService {
  ...
  async deleteAllSessionsExceptCurrent(tokenString: string, userId: string): Promise<DeleteResult> {
    ...
    return await this.sessionsRepository.deleteAllSessionsExceptCurrent(deviceId, userId);
  };
  ...
  async getSessionsByUserId(userId: string): Promise<ISessionViewModel[]> {
    const sessions = await this.sessionsRepository.getAllSessionsByUserId(userId);
  ...
  async updateSession(oldTokenString: string, newTokenString: string): Promise<boolean> {
    const oldMetadata = await this.jwtService.getRefreshTokenMetadata(oldTokenString);
    const newMetadata = await this.jwtService.getRefreshTokenMetadata(newTokenString);
    ...
    return await this.sessionsRepository.updateSession(oldDeviceId, newIssuedAt);
  };
