...
export class SessionsController {
  ...
  async deleteAllSessionsExceptCurrent(req: Request, res: Response) {
    ...
    const result = await this.sessionsService.deleteAllSessionsExceptCurrent(refreshTokenFromReq, userId);
    if (result) {
  ...
  async deleteSessionByDeviceId(req: Request, res: Response) {
    ...
    const result = await this.sessionsService.deleteSessionByDeviceId(deviceId, userId);
    if (result === true) {
      res.sendStatus(CodeResponsesEnum.No_content_204);
      return;
    };
